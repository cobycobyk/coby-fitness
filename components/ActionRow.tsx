...
const ActionRow = ({ title, screen, color, requiresPro, icon, vertical }: Props) => {
  ...
  return (
    <TouchableOpacity
      ...
      className={`flex m-2 flex-1 justify-center items-center py-6 rounded-lg space-x-2 ${vertical ? "flex-col" : "flex-row"}`} style={{ backgroundColor: lockedForProMembers ? "gray" : color }}>
